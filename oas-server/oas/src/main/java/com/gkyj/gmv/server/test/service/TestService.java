...
public class TestService {
    ...
    public List<TestModel> selectLoadMsgByPeriod(String startTime,String endTime){
        Select LoadMsgByPeriod = QueryBuilder.select().from("box").where(QueryBuilder.eq("circle_number",2)).and(QueryBuilder.gte("time",startTime)).and(QueryBuilder.lte("time",endTime)).allowFiltering();
