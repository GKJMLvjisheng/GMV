...
function resizable(chart1){
	...
	$(".highcharts-partfill-original").resizable({
		...
		onStartResize: function(e){			
			...
			rightStart = Number(resizeStart) + Number(rect_width);
			//确定拖动元素是哪一个
			number = $(this).parent().parent().attr("class").split(" ")[1].replace("highcharts-series-","");
			resize_left1 = $($(".highcharts-partfill-original")[number]).position().left; //甘特图的位置信息
			resize_left2 = $($("#dash" + number))[0].offsetLeft;	//虚线框的信息	
			resize_width2 = $($("#dash" + number))[0].offsetWidth;						
		},
		onResize: function(e){
			var $drag_object = $(this);
			...
			var resize_width = ($(".highcharts-partfill-original")[number].style.width).replace("px","");  //拖动后的宽度
			
			resize_left = $($(".highcharts-partfill-original")[number]).position().left;  //甘特图的位置信息
			...
			if(resize_location2 >= resize_location){
				...
				if(Number(resize_width) - Number(rect_width) < 0){
					...
					//移不出左边框
					if(resize_left <= resize_left2){
						resize_left = resize_left2;
						//$(this).attr('x',Number(rect_x)+5);
						$(this).attr('x', Math.abs(Number(resize_left)-356.86+1));
						$drag_object.attr('x2',rightStart);	//右边框不动
					}
				}else{
					//拖动右边框，左边框不动
					$drag_object.attr('x',resizeStart);		
					//移不出右边框
					if (resize_left + Number(resize_width) > resize_left2 + resize_width2 ){								 								
						resize_width = Math.abs(Number(resize_width2) - (Number(resize_left1) - Number(resize_left2)));
						$(".highcharts-partfill-original")[number].style.width = resize_width;
						$drag_object.attr('x',resizeStart);	//左边框不动	
					}
				}											
			}else{
				//鼠标向左移动
				if(Number(resize_width) - Number(rect_width) > 0){
					//拖动左边框，右边框不动					
					if(Number(resize_left1) - step < Number(resize_left2)){
						//拖出左侧虚线框
						var step1 = Number(resize_left1) - Number(resize_left2);
						$(this).attr('x',Number(resizeStart) - step1);
						var resizeWidth = Number(rect_width) + Number(resize_left1) - Number(resize_left2);//拖动后的宽度
						$(".highcharts-partfill-original")[number].style.width = resizeWidth;
									
					}else{
						$(this).attr('x',Number(resizeStart) - step);
			...
			//html显示resize后的数据
			chart1.series[number].points[0].shapeArgs.x = $drag_object.attr('x');
			if(Number(resize_width) > Number(resize_width2)){
				resize_width = resize_width2;
			}
			chart1.series[number].points[0].shapeArgs.width = (resizeWidth != undefined ? String(resizeWidth) : resizeWidth) || String(resize_width);
		...
		onStopResize: function (e) {
			...
			var resize_widthMax = ($(".highcharts-partfill-original")[number].style.width).replace("px","");  //拖动后的宽度
			
			if(resize_Elocation < resize_location){
				//鼠标向左移动
				if(Number(resize_widthMax) - Number(rect_width) > 0){
					//拖动左边框，右边框不动
					if(Number(resize_left1) - stepMax < Number(resize_left2)){
						//拖出左侧虚线框
						var step1 = Number(resize_left1) - Number(resize_left2);
						$(this).attr('x',Number(resizeStart) - step1);
						var resizeWidth = Number(rect_width) + Number(resize_left1) - Number(resize_left2);//拖动后的宽度
					...
					//移不出右边框
					if (resize_left + Number(resize_widthMax) > resize_left2 + resize_width2 ){
						resize_widthMax = Math.abs(Number(resize_width2) - (Number(resize_left1) - Number(resize_left2)));
						$(".highcharts-partfill-original")[number].style.width = resize_widthMax;
						$drag_object.attr('x',resizeStart);	//左边框不动
					}
				}												
			}else{
			...
			chart1.series[number].points[0].shapeArgs.x = $(this).attr('x');
			chart1.series[number].points[0].shapeArgs.width = (resizeWidth != undefined ? String(resizeWidth) : resizeWidth) || String(resize_widthMax);
